...
BOOL Getdetection(tensorflow::Tensor& encoded_locations,
                                std::vector<float>& boxRect) {
  const int width = 440;
  const int height = 440;
  auto locations_encoded = encoded_locations.flat<float>();
  if (locations_encoded.size() > 0){
    for (int pos = 0; pos < locations_encoded.size() / 4; ++pos) {
      ...
      boxRect.push_back(ymin * height);
      boxRect.push_back(xmin * width);
      boxRect.push_back(ymax * height);
      boxRect.push_back(xmax * width);
    }
    return YES;
    }
  else
