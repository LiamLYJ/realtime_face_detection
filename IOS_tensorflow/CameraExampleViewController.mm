...
const int wanted_input_width = 440;
const int wanted_input_height = 440;
const int wanted_input_channels = 3;
// const float input_mean = 117.0f;
// const float input_std = 1.0f;
// const std::string input_layer_name = "input";
//const std::string output_layer_name = "softmax1";
const std::string input_layer_name = "input_image";
//const std::string output_layer_name = "MobilenetV1/Box/softmax_3/Reshape_1";
const std::string output_layer_name = "final_bboxes";
...
- (void)runCNNOnFrame:(CVPixelBufferRef)pixelBuffer {
  ...
  if (tf_session.get()) {
    ...
    if (!run_status.ok()) {
      ...
      // with draw bboxes only
      tensorflow::Tensor& encoded_locations = outputs[0];
      std::vector<float> boxRect;
      BOOL get_results = Getdetection(encoded_locations, boxRect);
      if (get_results){
        dispatch_async(dispatch_get_main_queue(), ^(void) {
          [self setPredictionValues:boxRect];
        });
      }
    }
  }
...
- (void)setPredictionValues:(std::vector<float> ) boxRect {
  [self removeAllLabelLayers];
  [self addLabelLayerWithText: boxRect];
}
...
- (void)addLabelLayerWithText:(std::vector<float> ) boxRect{

  // draw rect on view_layer
  UIBezierPath *tmp_path = [UIBezierPath bezierPathWithRect:CGRectMake(0,0,0,0)];
  CAShapeLayer * mylayer = [CAShapeLayer layer];
  for( int i =0; i < boxRect.size() /4; i++){
    const float ymin = boxRect.at(i * 4 + 0);
    const float xmin = boxRect.at(i * 4 + 1);
    const float ymax = boxRect.at(i * 4 + 2);
    const float xmax = boxRect.at(i * 4 + 3);
    [tmp_path appendPath:[UIBezierPath bezierPathWithRect:CGRectMake(xmin, ymin, xmax-xmin, ymax-ymin)]];
//    mylayer.path = [UIBezierPath bezierPathWithRect:CGRectMake(xmin, ymin, xmax-xmin, ymax-ymin)].CGPath;
  }
  mylayer.path = tmp_path.CGPath;
  mylayer.fillColor = [UIColor clearColor].CGColor;
  mylayer.strokeColor = [UIColor redColor].CGColor;
  [[self.view layer] addSublayer:mylayer];
  [labelLayers addObject:mylayer];
